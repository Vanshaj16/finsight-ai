...
const ProfileModal = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/65 px-4 py-6 backdrop-blur-sm">
      <div className="card-surface relative w-full max-w-2xl p-6 text-white lg:p-7">
        <button
          type="button"
          onClick={onClose}
        ...
        <div className="grid gap-6 lg:grid-cols-[220px_1fr] lg:items-start">
          <div className="rounded-[28px] border border-white/8 bg-white/4 p-5 text-center">
            <div className="mx-auto flex h-24 w-24 items-center justify-center overflow-hidden rounded-full border border-white/12 bg-gradient-to-br from-[#6C63FF] to-[#4F9CFF] text-3xl font-semibold text-white shadow-[0_18px_40px_rgba(79,156,255,0.26)]">
              {form.profilePic ? (
            ...
            <p className="mt-4 text-lg font-semibold text-white">{form.name || user.name}</p>
            <p className="mt-1 text-sm text-white/50">{form.email || user.email}</p>
          ...
          <div>
            ...
            <h3 className="mt-3 text-3xl font-semibold text-white">Read and update your details</h3>
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-4">
              ...
              <div className="flex flex-wrap gap-3">
                <button type="submit" disabled={saving} className="primary-button inline-flex items-center gap-2 disabled:opacity-70">
                  <PencilLine size={16} />
                  {saving ? "Saving..." : "Save changes"}
                </button>
                <button type="button" onClick={onClose} className="secondary-button !text-white">
                  Close
                </button>
              </div>
            </form>
